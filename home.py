...
st.info("📱 **Mobile Camera with Focus Control** - Tap the video to focus, then click capture")

# Custom HTML camera with focus control
...
<head>
    <style>
        #video { width: 100%; max-width: 500px; height: 300px; border: 2px solid #ccc; }
        #canvas { display: none; }
        .camera-container { text-align: center; }
        .focus-ring { position: absolute; border: 2px solid #00ff00; border-radius: 50%; width: 60px; height: 60px; pointer-events: none; }
...
<body>
    ...
    <script>
        ...
        function showFocusRing(x, y) {
            ...
            document.querySelectorAll('.focus-ring').forEach(ring => ring.remove());
            ...
            video.parentElement.appendChild(focusRing);
            
            // Animate and remove
            setTimeout(() => {
                focusRing.style.transform = 'scale(0.8)';
                focusRing.style.opacity = '0.5';
            }, 100);
...
# JavaScript to receive captured image
components.html("""
<script>
...
# Fallback: Streamlit's camera input
st.markdown("---")
st.markdown("**Fallback option:**")
camera_image = st.camera_input("Take a picture of a barcode (if camera above doesn't work)")
